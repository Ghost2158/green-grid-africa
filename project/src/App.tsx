...
import { SolarOnboarding } from './components/auth/SolarOnboarding';
...
const AppContent: React.FC = () => {
  const { authState, logout, canAccessDashboard, updateProfile, closeProfileModal } = useAuth();
  ...
  const [showSolarOnboarding, setShowSolarOnboarding] = useState(false);
  // Show onboarding after profile completion
  React.useEffect(() => {
    if (authState.user && authState.user.profileCompleted && !showSolarOnboarding && !localStorage.getItem('solarOnboardingDone')) {
      setShowSolarOnboarding(true);
    }
  }, [authState.user, showSolarOnboarding]);

  // Update active tab when user logs in
  React.useEffect(() => {
    if (authState.user) {
      // Only set default dashboard if not currently on profile page
      if (activeTab !== 'profile') {
        console.log('Setting default dashboard for role:', authState.user.role, 'Current tab:', activeTab);
        switch (authState.user.role) {
          case 'user':
        ...
        console.log('Keeping profile page open, current tab:', activeTab);
      }
    }
  }, [authState.user]); // Remove activeTab from dependencies to prevent loops
  ...
  const handleProfileClick = () => {
    console.log('Profile button clicked, setting activeTab to profile');
  ...
  if (activeTab === 'profile') {
    ...
    DashboardComponent = () => <ProfilePage onBack={handleBackToDashboard} />;
  } else {
    currentDashboard = dashboardConfig[activeTab as keyof typeof dashboardConfig];
    DashboardComponent = currentDashboard.component;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="flex h-screen">
        <Sidebar 
          ...
          onTabChange={(tab) => {
            ...
            console.log('canAccessDashboard result:', canAccessDashboard(tab));
            if (canAccessDashboard(tab)) {
              console.log('Setting activeTab to:', tab);
              setActiveTab(tab);
            } else {
              console.log('Access denied for tab:', tab);
            }
          }}
        />
      ...
      <ProfileCompletionModal
        isOpen={authState.showProfileModal}
        onClose={closeProfileModal}
        onComplete={async (data) => {
          await updateProfile(data);
          // After profile completion, show solar onboarding
          setShowSolarOnboarding(true);
        }}
        isLoading={authState.isLoading}
      />
      {/* Solar System Onboarding Modal */}
      {showSolarOnboarding && (
        <SolarOnboarding
          onComplete={(systems) => {
            // Save systems to backend here (TODO: implement Supabase save)
            localStorage.setItem('solarOnboardingDone', 'true');
            setShowSolarOnboarding(false);
          }}
        />
      )}
    </div>
