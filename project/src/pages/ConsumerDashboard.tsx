...
export const ConsumerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    // For new users, setup is blank and all graphs are zero
    const blankSetup: UserSolarSetup = {
      panelCount: 0,
      panelCapacity: 0,
      totalCapacity: 0,
    ...
    setUserSetup(blankSetup);
    generatePredictions(blankSetup);
    setWeatherForecast([]);
  }, [timeHorizon]);

  const generatePredictions = (setup: UserSolarSetup) => {
    // For new users, all predictions are zero
  ...
  // For new users, weather forecast is blank
